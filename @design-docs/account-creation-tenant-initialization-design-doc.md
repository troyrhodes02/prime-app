...
| Element | Styling & Behavior |
|---|---|
| **OTP field** | Single text input: `w-full rounded-lg border border-gray-200 px-3 py-2.5 text-center text-lg font-medium tracking-widest focus:ring-2 focus:ring-blue-600 focus:border-transparent`. `inputMode="numeric"`, `autocomplete="one-time-code"`. Accepts variable-length codes (Supabase OTP codes may exceed 6 digits). |
| **Verify button** | "Verify" — same styling as Continue button on signup. Disabled until input is non-empty. |
  ...
  <div className="w-full max-w-sm rounded-xl bg-white p-8 shadow-sm">
    ...
    <form onSubmit={handleVerify} className="mt-6 space-y-3">
      ...
      <button
        ...
        className="w-full rounded-lg bg-blue-600 py-2.5 text-sm
...
| Field | Type | Required | Notes |
|---|---|---|---|
| OTP code | Single text input | Yes | Numeric only. `inputMode="numeric"`, `autocomplete="one-time-code"`. Accepts variable-length codes (Supabase codes may exceed 6 digits). Paste support. |

#### Validation

- **Empty code:** Verify button disabled
- **Invalid code:** Inline error below input: "Invalid code. Check your email and try again." — `text-xs text-red-600 mt-2 text-center`
...
Supabase OTP emails include both a numeric code and a clickable magic link. The user can either enter the code on this screen or click the link in the email (which routes through `/api/auth/callback`). Both paths complete authentication identically.
...
- **Verification failure:** Show inline error, clear OTP field, re-focus input
- **Resend:** 60-second cooldown with visible countdown. Toast on resend: "Code sent"
- **Back:** Returns to signup with email pre-filled
- **Paste support:** Pasting a code populates the field
- **Focus:** OTP input auto-focused on mount
...
| Verify | `0-9` | Enter digits into code field |
| Verify | `Ctrl/Cmd+V` | Paste code into field |
| Verify | `Enter` | Submit verification (when code is non-empty) |
...
- **OTP input:** Full-width text field — no mobile size adaptation needed
...
| `OtpInput` | Verify | Single text field for variable-length OTP codes. Numeric input mode, paste support. |
